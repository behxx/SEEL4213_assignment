...
class DateAxisItem(AxisItem):
    ...
    def tickStrings(self, values, scale, spacing):
        ...
        for x in values:
            try:
                # Convert the timestamp (x) to a datetime object in UTC timezone
                dt_utc = datetime.fromtimestamp(x, tz=pytz.utc)

                # Convert the datetime object to the desired timezone (Kuala Lumpur timezone)
                dt_kl = dt_utc.astimezone(kl_timezone)

                # Format the datetime object based on the spacing
                ...
                formatted_time = dt_kl.strftime(fmt)
